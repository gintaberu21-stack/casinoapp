...
スマホ2台のオンライン対戦まで実装済みです。Express + WebSocketのホスト権威方式で、ID検索、申し込み、承認、対戦開始、ターン、手札、必殺技、チップ、ラウンド結果を同期します。MongoDBがあれば在席と試合結果を保存し、ローカルではメモリ保存で動きます。文化祭向けの本番環境は、サインイン・合言葉なしでURLからすぐ遊べます。
